scoreboard players set @s promote_queen 0
scoreboard players enable @s promote_queen
say QUEEN
...
$execute if score #cur_color temp matches 1 run data modify storage chess:boards boards.$(board_id).squares[$(to_move_pos)] set value 5
$execute if score #cur_color temp matches 2 run data modify storage chess:boards boards.$(board_id).squares[$(to_move_pos)] set value 11
